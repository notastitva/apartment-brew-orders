...
const PAGE = document.body.dataset.page || (
  window.location.pathname.includes('subscribe') ? 'SUBSCRIBE' :
  window.location.pathname.includes('pass') ? 'PASS' :
...
let availablePassTiers = [
  { id: 'PASS-01', name: 'Weekend 4-Pack Pass (4 Drops)', bottles: 16, drops: 4, bottlesPerDrop: 4, unitPrice: 3199, price: 3199, badge: 'Most Popular • Save 11%', desc: '16x 200ml bottles (4 bottles/drop across 4 scheduled weekend drops)' },
  { id: 'PASS-02', name: 'Duo Discovery Pass (4 Drops)', bottles: 8, drops: 4, bottlesPerDrop: 2, unitPrice: 1699, price: 1699, badge: 'Discovery Sampler', desc: '8x 200ml bottles (2 bottles/drop across 4 scheduled weekend drops)' },
  { id: 'PASS-03', name: 'Corporate Team Pass (4 Drops)', bottles: 40, drops: 4, bottlesPerDrop: 10, unitPrice: 6480, price: 6480, badge: 'Office Team • Save 10%', desc: '40x 200ml bottles (10 bottles/drop across 4 Friday office drops)' },
  { id: 'PASS-04', name: 'Mega Weekender Pass (4 Drops)', bottles: 24, drops: 4, bottlesPerDrop: 6, unitPrice: 4199, price: 4199, badge: 'Ultimate Value • Save 13%', desc: '24x 200ml bottles (6 bottles/drop across 4 scheduled weekend drops)' }
];
let selectedPassTier = null;
...
function renderPassPacks() {
  const container = document.getElementById('passPacks');
  if (!container) return;
  let html = '';
  availablePassTiers.forEach(p => {
    const isSelected = selectedPassTier && (selectedPassTier.id === p.id);
    html += `
      <div class="pack-option ${isSelected ? 'active' : ''}" onclick="selectPassTier('${p.id}')">
...
function selectPassTier(id) {
  playHapticTap('click');
  selectedPassTier = availablePassTiers.find(p => p.id === id) || null;
...
function setPassMode(mode) {
  playHapticTap('click');
  const tabBuy = document.getElementById('tabBuyPass');
  const tabManage = document.getElementById('tabManagePass');
  const buyView = document.getElementById('orderFormView');
  const manageView = document.getElementById('managePassView');
  if (tabBuy) tabBuy.classList.toggle('active', mode === 'BUY');
  if (tabManage) tabManage.classList.toggle('active', mode === 'MANAGE');
  if (buyView) buyView.style.display = (mode === 'BUY') ? 'block' : 'none';
  if (manageView) manageView.style.display = (mode === 'MANAGE') ? 'block' : 'none';
}

function lookupActivePass(passId) {
  const input = document.getElementById('passLookupInput');
  const pId = (passId || (input ? input.value.trim().toUpperCase() : '')).trim();
  const msgEl = document.getElementById('passLookupMsg');
  const cardEl = document.getElementById('passDetailsCard');
  if (!pId) {
    if (msgEl) {
      msgEl.textContent = 'Please enter a valid Pass ID (e.g. TABC-PASS-104).';
      msgEl.style.display = 'block';
      msgEl.style.background = 'rgba(230,57,70,0.2)';
      msgEl.style.color = '#ff8fa3';
    }
    return;
  }
  if (msgEl) {
  ...
  const url = `${CONFIG.googleSheetEndpoint}?action=track&orderId=${encodeURIComponent(pId)}`;
  fetch(url)
    .then(r => r.json())
    .then(data => {
      if (data && data.status === 'success' && data.order) {
        ...
        if (msgEl) {
          msgEl.textContent = `✕ No active coffee pass found with ID "${pId}". Please verify your Pass ID.`;
          msgEl.style.background = 'rgba(230,57,70,0.2)';
          msgEl.style.color = '#ff8fa3';
        }
    ...
    .catch(() => {
      if (msgEl) {
        msgEl.textContent = '⚠️ Network error querying pass status.';
        msgEl.style.background = 'rgba(230,57,70,0.2)';
        msgEl.style.color = '#ff8fa3';
      }
    });
...
let selectedLotToSwitch = null;

function selectHarvestLotForSwitch(lotFullName, lotName) {
  playHapticTap('click');
  selectedLotToSwitch = lotFullName;
  document.querySelectorAll('.harvest-switch-card').forEach(function(card) {
    if (card.dataset.lotFullName === lotFullName) {
      card.classList.add('active');
    } else {
      card.classList.remove('active');
    }
  });
  const btnText = document.getElementById('btnConfirmHarvestSwitchText');
  if (btnText) {
    btnText.textContent = 'Switch Next Drop to ' + lotName + ' →';
  }
}

function confirmHarvestSwitch() {
  if (!selectedLotToSwitch) {
    alert('Please tap one of the single-estate harvest cards above to select it.');
    return;
  }
  handleSubscriptionAction('SWITCH_HARVEST', selectedLotToSwitch);
}

function renderPassDetailsCard(order) {
  const cardEl = document.getElementById('passDetailsCard');
  if (!cardEl) return;
  ...
  let unfulfilledDrop = null;

  dropsData.forEach(function(d, idx) {
    const stLower = d.status.toLowerCase();
    const isDelivered = stLower === 'delivered';
    const isDropPaused = isPaused || stLower.includes('pause');
    const isSkipped = stLower.includes('skip') || stLower.includes('roll');
    const isDropCancelled = isCancelled || stLower.includes('cancel');
    const isNext = !isDelivered && !isDropPaused && !isDropCancelled && (idx === 0 || dropsData[idx - 1].status.toLowerCase() === 'delivered');

    if (!isDelivered && !unfulfilledDrop && !isDropCancelled) {
      unfulfilledDrop = d;
    }
    ...
    if (isDelivered) {
      ...
      dColor = 'var(--accent)';
      dIcon = '☕';
      dStatusLabel = 'QUEUED FOR EXTRACTION';
    } else if (isSkipped) {
      dColor = '#f39c12';
      dIcon = '⏭️';
      dStatusLabel = 'ROLLED FORWARD';
    } else if (isDropCancelled) {
      dColor = '#ff8fa3';
      dIcon = '✕';
      dStatusLabel = 'CANCELLED';
    }

    let dateDisplay = d.dropDate;
    if (isDropPaused || (typeof d.dropDate === 'string' && d.dropDate.toLowerCase().includes('not yet available'))) {
      dateDisplay = '<span style="color: #f39c12; font-weight: 700;">Not yet available (Paused)</span>';
    }

    const trackBtn = isDropCancelled ? '' : `<a href="/track?orderId=${encodeURIComponent(d.dropId)}" class="btn-secondary" style="font-size: 0.72rem; padding: 6px 14px; text-decoration: none; border-radius: 6px; white-space: nowrap;">${isDelivered ? 'Rate & Track &rarr;' : 'Track Drop &rarr;'}</a>`;

    dropsHtml += `
      <div style="background: #151412; border: 1px solid var(--card-border); border-radius: 10px; padding: 12px 14px; display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; flex-wrap: wrap; gap: 8px;">
        <div style="display: flex; align-items: center; gap: 10px;">
          <div style="width: 28px; height: 28px; min-width: 28px; border-radius: 50%; background: rgba(255,255,255,0.05); border: 1px solid ${dColor}; color: ${dColor}; display: flex; align-items: center; justify-content: center; font-weight: 900; font-size: 0.82rem;">
            ${dIcon}
          </div>
          <div>
            ...
            <div style="font-size: 0.72rem; color: var(--text-muted); margin-top: 2px;">${dateDisplay} &bull; ${d.bean} &bull; <strong style="color: ${dColor};">${dStatusLabel}</strong></div>
          </div>
        </div>
        ${trackBtn}
      </div>`;
  ...
  const currentUpcomingLot = (unfulfilledDrop ? unfulfilledDrop.bean : order.bean) || 'Single-Estate Selection';
  selectedLotToSwitch = null;

  let harvestCardsHtml = '';
  if (availableLots && availableLots.length > 0) {
    availableLots.filter(l => l.isActive !== false).forEach(function(lot) {
      const fullLotVal = `${lot.name} (${lot.process})`;
      const isCurrent = (lot.name === currentUpcomingLot || currentUpcomingLot.includes(lot.name));
      const emoji = (lot.emojis && lot.emojis.length > 0) ? lot.emojis[0] : '☕';

      harvestCardsHtml += `
        <div class="harvest-switch-card ${isCurrent ? 'active' : ''}" data-lot-full-name="${fullLotVal}" onclick="selectHarvestLotForSwitch('${fullLotVal}', '${lot.name}')">
          <div style="display: flex; justify-content: space-between; align-items: flex-start;">
            <span style="font-weight: 800; font-size: 0.84rem; color: var(--text);">${lot.name}</span>
            <span style="font-size: 0.95rem;">${emoji}</span>
          </div>
          <div style="font-size: 0.7rem; color: var(--accent); font-weight: 700; margin: 3px 0;">${lot.process}</div>
          <div style="font-size: 0.68rem; color: var(--text-muted); line-height: 1.35;">${lot.notes || ''}</div>
          ${isCurrent ? '<div style="margin-top: 6px; font-size: 0.62rem; font-weight: 800; color: var(--accent); letter-spacing: 0.5px; text-transform: uppercase;">● CURRENT LOT</div>' : ''}
        </div>
      `;
    });
  }

  const curNextDate = unfulfilledDrop ? unfulfilledDrop.dropDate : (order.dropDate || 'Upcoming Drop');
  let rolledPreviewDate = 'Next Cycle (+7 Days)';
  const curDObj = new Date(curNextDate);
  if (!isNaN(curDObj.getTime())) {
    curDObj.setDate(curDObj.getDate() + 7);
    rolledPreviewDate = curDObj.toLocaleDateString('en-IN', { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' });
  }

  let actionControlsHtml = '';
  if (!isCancelled && remaining > 0) {
    actionControlsHtml = `
      <div style="margin-top: 20px; border-top: 1.5px dashed rgba(212,163,115,0.35); padding-top: 18px;">
        <div style="font-size: 0.85rem; font-weight: 800; color: var(--accent); letter-spacing: 0.6px; text-transform: uppercase; margin-bottom: 12px; display: flex; align-items: center; gap: 8px;">
          <span>🔄</span> Subscription Lifecycle &amp; Drop Controls
        </div>

        <!-- MODULE 1: Interactive Harvest Switcher -->
        <div class="pass-management-module" style="background: rgba(0,0,0,0.3); border: 1px solid var(--card-border); border-radius: 12px; padding: 14px; margin-bottom: 14px;">
          <div class="module-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
            <div style="display: flex; align-items: center; gap: 8px;">
              <span style="font-size: 1.1rem;">🌱</span>
              <div>
                <div style="font-size: 0.85rem; font-weight: 800; color: var(--text);">Switch Upcoming Harvest Lot</div>
                <div style="font-size: 0.72rem; color: var(--text-muted);">Select a curated estate lot for your next extraction:</div>
              </div>
            </div>
            <span class="standing-order-pill" style="font-size: 0.65rem; background: rgba(212,163,115,0.15); color: var(--accent); border: 1px solid rgba(212,163,115,0.4); padding: 3px 8px; border-radius: 6px;">FREE SWAP</span>
          </div>

          <div class="harvest-switch-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 8px; margin: 10px 0;">
            ${harvestCardsHtml}
          </div>

          <button type="button" class="btn btn-pay" id="btnConfirmHarvestSwitch" style="width: 100%; margin-top: 6px;" onclick="confirmHarvestSwitch()">
            <span id="btnConfirmHarvestSwitchText">☕ Confirm Harvest Switch &rarr;</span>
          </button>
        </div>

        <!-- MODULE 2: Cascading Schedule Rollover -->
        <div class="pass-management-module" style="background: rgba(0,0,0,0.3); border: 1px solid var(--card-border); border-radius: 12px; padding: 14px; margin-bottom: 14px;">
          <div class="module-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
            <div style="display: flex; align-items: center; gap: 8px;">
              <span style="font-size: 1.1rem;">⏭️</span>
              <div>
                <div style="font-size: 0.85rem; font-weight: 800; color: var(--text);">Cascading Schedule Rollover</div>
                <div style="font-size: 0.72rem; color: var(--text-muted);">Skip next drop; subsequent deliveries automatically shift forward by 1 week</div>
              </div>
            </div>
          </div>

          <div class="module-preview-pill" style="display: flex; align-items: center; justify-content: space-between; background: #151412; border: 1px dashed var(--card-border); border-radius: 8px; padding: 8px 12px; font-size: 0.76rem; color: var(--text-muted); margin-top: 6px;">
            <span>Current: <strong style="color: var(--text);">${curNextDate}</strong></span>
            <span style="color: var(--accent); font-weight: 800;">&rarr;</span>
            <span>Rolled Forward: <strong style="color: var(--accent);">${rolledPreviewDate}</strong></span>
          </div>

          <button type="button" class="btn btn-secondary" style="border-color: rgba(212,163,115,0.45); color: var(--accent); width: 100%; margin-top: 10px;" onclick="handleSubscriptionAction('SKIP')">
            <span>⏭️ Rollover Next Drop (+1 Week) &rarr;</span>
          </button>
        </div>

        <!-- MODULE 3: Vacation Hold & Status (Pause / Resume) -->
        <div class="pass-management-module" style="background: rgba(0,0,0,0.3); border: 1px solid var(--card-border); border-radius: 12px; padding: 14px; margin-bottom: 14px;">
          <div class="module-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
            <div style="display: flex; align-items: center; gap: 8px;">
              <span style="font-size: 1.1rem;">${isPaused ? '⏸️' : '🏖️'}</span>
              <div>
                <div style="font-size: 0.85rem; font-weight: 800; color: var(--text);">${isPaused ? 'Subscription Currently Paused' : 'Vacation Hold / Pause'}</div>
                <div style="font-size: 0.72rem; color: var(--text-muted);">
                  ${isPaused ? 'Upcoming dates are held as \"Not yet available\". Resume to reschedule all remaining drops.' : 'Temporarily hold deliveries while away. Your bottle credits remain 100% safe.'}
                </div>
              </div>
            </div>
          </div>
          ...
          ${isPaused ? `
            <button type="button" class="btn btn-pay" style="width: 100%; margin-top: 8px;" onclick="handleSubscriptionAction('RESUME')">
              <span>▶️ Resume Subscription &amp; Reschedule Remaining Drops &rarr;</span>
            </button>
          ` : `
            <button type="button" class="btn btn-secondary" style="border-color: rgba(243, 156, 18, 0.5); color: #f39c12; width: 100%; margin-top: 8px;" onclick="if(confirm('Pause subscription deliveries? All upcoming drop dates will be held as \"Not yet available\" until you resume.')){handleSubscriptionAction('PAUSE');}">
              <span>⏸️ Pause Subscription (Vacation Hold)</span>
            </button>
          `}

          <!-- Danger Zone: Cancel Subscription -->
          <div style="text-align: center; margin-top: 14px; padding-top: 10px; border-top: 1px dashed rgba(255,255,255,0.06);">
            <button type="button" style="background: none; border: none; color: var(--text-dim); font-size: 0.74rem; cursor: pointer; text-decoration: underline;" onclick="if(confirm('Are you sure you want to cancel your coffee pass subscription? All remaining unfulfilled drops will be cancelled.')){handleSubscriptionAction('CANCEL');}">
              Cancel Subscription
            </button>
          </div>
        </div>
  ...
  cardEl.innerHTML = `
    <div class="cold-chain-manifest-card" style="margin-top: 14px;">
      <!-- Left & Right Circular Ticket Cutouts -->
      <div class="manifest-cutout-left"></div>
      <div class="manifest-cutout-right"></div>

      <div class="manifest-header">
        <div style="display: flex; align-items: center; gap: 8px;">
          <span style="font-size: 0.68rem; font-weight: 800; color: var(--accent); letter-spacing: 0.8px; text-transform: uppercase;">✈️ COLD-CHAIN SUBSCRIPTION MANIFEST</span>
        </div>
        <span class="manifest-stamp">★ 4-DROP CRAFT PASS ★</span>
      </div>

      <div class="manifest-body">
        <div class="tracker-summary-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px; flex-wrap: wrap; gap: 8px;">
          <div style="display: flex; align-items: center; gap: 8px;">
            ${statusBadge}
            <span style="font-size: 0.7rem; font-weight: 800; color: var(--text); background: rgba(255,255,255,0.06); padding: 4px 10px; border-radius: 8px;">
              ${remaining} OF ${dropsTotal} DROPS REMAINING
            </span>
          </div>
          <span class="tracker-order-id" style="font-family: monospace; font-size: 1.05rem; font-weight: 900; color: var(--accent);">${order.orderId}</span>
        </div>

        <div class="tracker-details-list">
          <div class="tracker-detail-row">
            <span>Pass Tier:</span>
            <strong>${order.pack || 'Weekend 4-Pack Pass (4 Drops)'}</strong>
          </div>
          <div class="tracker-detail-row">
            <span>Subscriber:</span>
            <strong>${order.customerName || 'Valued Subscriber'}</strong>
          </div>
          <div class="tracker-detail-row">
            <span>Delivery Window:</span>
            <strong style="color: var(--accent);">${order.deliveryWindow || 'Saturday Morning (8:00 AM – 11:00 AM)'}</strong>
          </div>
          <div class="tracker-detail-row">
            <span>Destination:</span>
            <strong>${order.deliveryAddress} (${order.dropInstructions || 'Deliver directly to door/desk'})</strong>
          </div>
          <div class="tracker-detail-row">
            <span>Current Harvest:</span>
            <strong style="color: var(--accent);">${order.bean || 'Single-Estate Selection'}</strong>
          </div>
        </div>

        <div style="font-size: 0.78rem; font-weight: 800; color: var(--accent); margin: 16px 0 8px; text-transform: uppercase; letter-spacing: 0.5px;">
          📦 Discrete Drop Schedule &amp; Tracking IDs
        </div>
        ${dropsHtml}

        ${actionControlsHtml}

        <div class="manifest-barcode-row" style="margin-top: 14px;">
          <div class="manifest-temp-log">❄️ SUB-4°C FLASH CHILLED &bull; ZERO PRESERVATIVES</div>
          <div style="font-size: 0.65rem; font-family: monospace; color: var(--text-dim); letter-spacing: 1px;">AUTONOMOUS PASS LEDGER</div>
        </div>
      </div>
    </div>`;
...
function handleSubscriptionAction(actionType, customPayload) {
  playHapticTap('click');
  ...
  let newHarvest = customPayload || '';

  if (actionType === 'SWITCH_HARVEST' && !newHarvest) {
    const sel = document.getElementById('selectNewHarvest');
    newHarvest = sel ? sel.value : '';
    if (!newHarvest) {
      alert('Please select a coffee lot to switch to.');
      return;
    }
  }
  ...
  if (actionType === 'SKIP' || actionType === 'ROLLOVER') {
    subAction = 'SKIP';
    waitText = '⏳ Rolling drop schedule forward by 1 week...';
    successText = '✓ Upcoming drop and subsequent drops rolled forward by 1 week.';
  } else if (actionType === 'PAUSE') {
    subAction = 'PAUSE';
    waitText = '⏳ Pausing subscription deliveries...';
    successText = '✓ Subscription paused! Upcoming dates set to "Not yet available".';
  } else if (actionType === 'RESUME') {
    subAction = 'RESUME';
    waitText = '⏳ Rescheduling remaining drops from current date...';
    successText = '✓ Subscription resumed! Remaining drops rescheduled starting from next available delivery date.';
  } else if (actionType === 'CANCEL') {
    subAction = 'CANCEL';
    waitText = '⏳ Cancelling subscription...';
    successText = '✓ Subscription cancelled. Remaining unscheduled drops have been cancelled.';
  } else if (actionType === 'SWITCH_HARVEST') {
    waitText = `⏳ Updating upcoming drop coffee harvest to ${newHarvest}...`;
    successText = `✓ Next drop single-estate harvest updated to ${newHarvest}!`;
  }
  ...
  // Management controls (Exclusively in pass.html)
  let actionControlsHtml = '';
  if (!isCancelled && remaining > 0) {
    actionControlsHtml = `
      <div style="margin-top: 18px; border-top: 1.5px dashed rgba(212,163,115,0.35); padding-top: 16px;">
        <div style="font-size: 0.82rem; font-weight: 800; color: var(--accent); letter-spacing: 0.5px; text-transform: uppercase; margin-bottom: 10px;">
          🔄 Manage Subscription Drops
        </div>
        <!-- 1. Swap Lot Control -->
        <div style="display: flex; gap: 8px; margin-bottom: 10px; flex-wrap: wrap;">
          <select id="selectNewHarvest" style="flex: 1; min-width: 200px; padding: 10px 12px; background: #141312; border: 1px solid var(--card-border); border-radius: 8px; color: var(--text); font-size: 0.82rem; font-weight: 600;">
            ${lotOptionsHtml}
          </select>
          <button type="button" class="btn btn-pay" style="width: auto; padding: 10px 18px; font-size: 0.82rem; font-weight: 700; white-space: nowrap;" onclick="handleSubscriptionAction('SWITCH_HARVEST')">
            <span>Update Coffee Lot</span>
          </button>
        </div>
        <!-- 2. Skip / Rollover Drop Control (Cascading) -->
        <div style="display: flex; gap: 8px; margin-bottom: 10px;">
          <button type="button" class="btn btn-secondary" style="flex: 1; padding: 10px 16px; font-size: 0.82rem; font-weight: 700;" onclick="handleSubscriptionAction('SKIP')">
            <span>⏭️ Rollover Next Drop (+1 Week / Shifts Following Drops)</span>
          </button>
        </div>
        <!-- 3. Pause / Resume Subscription -->
        <div style="display: flex; gap: 8px; margin-bottom: 10px;">
          ${isPaused ? `
            <button type="button" class="btn btn-pay" style="flex: 1; padding: 10px 16px; font-size: 0.82rem; font-weight: 700;" onclick="handleSubscriptionAction('RESUME')">
              <span>▶️ Resume Subscription (Reschedule Remaining Drops from Today)</span>
            </button>
          ` : `
            <button type="button" class="btn btn-secondary" style="flex: 1; padding: 10px 16px; font-size: 0.82rem; font-weight: 700; border-color: rgba(243,156,18,0.5); color: #f39c12;" onclick="handleSubscriptionAction('PAUSE')">
              <span>⏸️ Pause Subscription (Hold Upcoming Deliveries)</span>
            </button>
          `}
        </div>
        <!-- 4. Cancel Subscription -->
        <div style="text-align: center; margin-top: 12px;">
          <button type="button" style="background: none; border: none; color: var(--error-light); font-size: 0.74rem; cursor: pointer; text-decoration: underline;" onclick="if(confirm('Are you sure you want to cancel your coffee pass subscription? All remaining unfulfilled drops will be cancelled.')){handleSubscriptionAction('CANCEL');}">
            Cancel Subscription
          </button>
        </div>
        <div id="subActionInlineMsg" class="track-status-msg" style="display: none; margin-top: 10px; text-align: center;"></div>
...
function validateWizardStep(stepNum) {
  if (stepNum === 1) {
    ...
    else activePack = (PAGE === 'CORPORATE' || PAGE === 'OFFICE') ? selectedB2bPack : selectedB2cPack;
    const errPack = document.getElementById('errPackSelection');
    if (!activePack) {
      if (errPack) {
...
function calculateSubtotal() {
  ...
  if (PAGE === 'SUBSCRIBE' || PAGE === 'PASS') {
    return (selectedPassTier && selectedPassTier.unitPrice ? selectedPassTier.unitPrice : 0) * qty;
  }
...
async function handleOrderSuccess(paymentId, statusText) {
  ...
  const btnGoManagePass = document.getElementById('btnGoManagePass');
  if (btnGoManagePass) {
    btnGoManagePass.href = `/pass?id=${encodeURIComponent(orderId)}`;
  }
  const btnGoTrackDrop = document.getElementById('btnGoTrackDrop');
  if (btnGoTrackDrop) {
...
if (btnGoToPassPortal) {
btnGoToPassPortal.href = `/pass?id=${encodeURIComponent(order.masterPassId || order.orderId)}`;
}
} else {
passRedirectCard.style.display = 'none';
}
}
if (subStatusBadge) {
...
function selectBitternessStar(val) {
  feedbackBitterness = parseInt(val, 10) || 3;
  document.querySelectorAll('.bitter-star-btn').forEach(function(b, idx) {
    if (idx < feedbackBitterness) b.classList.add('active');
    else b.classList.remove('active');
  });
...
function selectClarityStar(val) {
  feedbackClarity = parseInt(val, 10) || 5;
  document.querySelectorAll('.clarity-star-btn').forEach(function(b, idx) {
    if (idx < feedbackClarity) b.classList.add('active');
    else b.classList.remove('active');
  });
...
function renderOrderFeedbackSection(order) {
  ...
  if (existing) {
    // State B: Customer has already rated this specific order
    ...
    const bitterStars = '★'.repeat(Math.max(1, Math.min(5, bitterNum))) + '☆'.repeat(Math.max(0, 5 - bitterNum));
    const clarityStars = '★'.repeat(Math.max(1, Math.min(5, clarityNum))) + '☆'.repeat(Math.max(0, 5 - clarityNum));
    const dateStr = existing.timestamp ? (' • ' + new Date(existing.timestamp).toLocaleDateString()) : '';

    container.innerHTML = 
      '<div class="feedback-submitted-card">' +
        '<div style="font-size: 0.72rem; font-weight: 800; color: var(--accent); letter-spacing: 1px; text-transform: uppercase;">' +
          '✓ Your Sensory Rating for this Brew' +
        '</div>' +
        '<div class="submitted-stars-display">' + stars + ' (' + overallNum + '/5 Overall)</div>' +
        '<div class="submitted-dimensions-list">' +
          '<div class="submitted-dimension-item">' +
            '<span class="submitted-dimension-label">Overall Brew:</span>' +
            '<span class="submitted-dimension-val">' + stars + ' (' + overallNum + '/5)</span>' +
          '</div>' +
          '<div class="submitted-dimension-item">' +
            '<span class="submitted-dimension-label">Bitterness:</span>' +
            '<span class="submitted-dimension-val">' + bitterStars + ' (' + (bitternessDescriptions[bitterNum] || (bitterNum + '/5')) + ')</span>' +
          '</div>' +
          '<div class="submitted-dimension-item">' +
            '<span class="submitted-dimension-label">Notes Clarity:</span>' +
            '<span class="submitted-dimension-val">' + clarityStars + ' (' + (clarityDescriptions[clarityNum] || (clarityNum + '/5')) + ')</span>' +
          '</div>' +
          (existing.notes ? (
            '<div class="submitted-dimension-item" style="border-top: 1px solid var(--card-border); padding-top: 6px; margin-top: 2px;">' +
              '<span class="submitted-dimension-label">Barista Notes:</span>' +
              '<span class="submitted-dimension-val" style="color: var(--text); font-weight: 600;">' + existing.notes + '</span>' +
            '</div>'
          ) : '') +
        '</div>' +
  ...
  // State A: Order is delivered but not yet rated
  feedbackOverall = 5;
    ...
    '<div class="feedback-card">' +
      ...
      '<div class="feedback-dimension-group">' +
        ...
        '<div class="feedback-dimension-row">' +
          '<div class="feedback-dimension-header">' +
            '<span class="feedback-dimension-title">1. Overall Brew Experience</span>' +
            '<span class="feedback-dimension-value-tag" id="overallValTag">5 / 5 Stars</span>' +
          '</div>' +
          '<div class="rating-stars-row" style="margin: 4px 0 2px;">' +
            '<button type="button" class="rating-star-btn star-btn active" onclick="selectOverallStar(1)">★</button>' +
            '<button type="button" class="rating-star-btn star-btn active" onclick="selectOverallStar(2)">★</button>' +
            '<button type="button" class="rating-star-btn star-btn active" onclick="selectOverallStar(3)">★</button>' +
            '<button type="button" class="rating-star-btn star-btn active" onclick="selectOverallStar(4)">★</button>' +
            '<button type="button" class="rating-star-btn star-btn active" onclick="selectOverallStar(5)">★</button>' +
          '</div>' +
        '</div>' +
        ...
        '<div class="feedback-dimension-row">' +
          '<div class="feedback-dimension-header">' +
            '<span class="feedback-dimension-title">2. Bitterness Level</span>' +
            '<span class="feedback-dimension-value-tag" id="bitterValTag">3/5 • Balanced</span>' +
          '</div>' +
          '<div class="rating-stars-row" style="margin: 4px 0 2px;">' +
            '<button type="button" class="rating-star-btn bitter-star-btn active" onclick="selectBitternessStar(1)">★</button>' +
            '<button type="button" class="rating-star-btn bitter-star-btn active" onclick="selectBitternessStar(2)">★</button>' +
            '<button type="button" class="rating-star-btn bitter-star-btn active" onclick="selectBitternessStar(3)">★</button>' +
            '<button type="button" class="rating-star-btn bitter-star-btn" onclick="selectBitternessStar(4)">★</button>' +
            '<button type="button" class="rating-star-btn bitter-star-btn" onclick="selectBitternessStar(5)">★</button>' +
          '</div>' +
        '</div>' +
        ...
        '<div class="feedback-dimension-row">' +
          '<div class="feedback-dimension-header">' +
            '<span class="feedback-dimension-title">3. Tasting Notes Clarity</span>' +
            '<span class="feedback-dimension-value-tag" id="clarityValTag">5/5 • Crystalline / Complex</span>' +
          '</div>' +
          '<div class="rating-stars-row" style="margin: 4px 0 2px;">' +
            '<button type="button" class="rating-star-btn clarity-star-btn active" onclick="selectClarityStar(1)">★</button>' +
            '<button type="button" class="rating-star-btn clarity-star-btn active" onclick="selectClarityStar(2)">★</button>' +
            '<button type="button" class="rating-star-btn clarity-star-btn active" onclick="selectClarityStar(3)">★</button>' +
            '<button type="button" class="rating-star-btn clarity-star-btn active" onclick="selectClarityStar(4)">★</button>' +
            '<button type="button" class="rating-star-btn clarity-star-btn active" onclick="selectClarityStar(5)">★</button>' +
          '</div>' +
        '</div>' +
      '</div>' +

      '<textarea id="feedbackCommentInput" class="feedback-textarea" rows="2" placeholder="Any specific feedback on acidity, ice temperature, or mouthfeel (optional)..."></textarea>' +
      '<button type="button" class="btn-submit-feedback" id="btnSubmitFeedback" onclick="submitFeedbackAction(\'' + order.orderId + '\', \'' + (order.customerName || '') + '\', \'' + (order.bean || '') + '\', \'' + (order.orderType || 'B2C') + '\')">' +
        '<span>Submit Calibration Rating &rarr;</span>' +
      '</button>' +
    '</div>';
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (PAGE === 'PERSONAL' || PAGE === 'ORDER') {
    ...
    fetchLiveConfig();
    const pId = urlParams.get('passId') || urlParams.get('id');
    if (pId) {
      const inp = document.getElementById('passLookupInput');
      if (inp) inp.value = pId;
      lookupActivePass(pId);
    }
  } else if (PAGE === 'ORDERS') {
