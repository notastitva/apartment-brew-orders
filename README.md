...
The Apartment Brew Co. operates an asset-light, pre-order only micro-roastery in Gurugram, India:

* Hot Extraction & Thermal Flash-Chill: Coffee is brewed hot at 92–94°C to dissolve delicate floral aromatic oils and fruit sugars, then instantly flash-chilled over ice to 4°C to lock in volatile flavor molecules before oxidation can occur.  
...
### **1.2 Dual Delivery Model & Roastery Cutoffs**
...
| Freshness Guide | guide.html | Science of flash-chilling, 48-hour freshness timeline, and cold storage/serving rituals |
| Craft & Philosophy | about.html | In-depth roastery story, small-batch hand-brewing philosophy, and direct trade farm sourcing |
...
* Stage 2 (Brewing & Chilling): Brewing, Roasting, Extracting, Chilling, Prep \-\> Highlights Step 2  
...
* Col K (Notes): Special Notes & Roast Preferences banner  
* Col L (Status): Controls 4-step timeline stepper and status badge  
* Col M (Lead Owner): Internal roastery assignee

---

## 7\. Operational SOP for Roastery Fulfillment
...
* 1\. Daily & Weekly Order Lifecycle:  
  ...
  * Friday 10:00 PM: B2C Saturday drop cutoff. Roast and bottle Saturday pre-orders.  
  * Saturday Morning: Cold-chain drop for Saturday residential pre-orders.  
  * Saturday 10:00 PM: B2C Sunday drop cutoff. Roast and bottle Sunday pre-orders.  
  * Sunday Morning: Cold-chain drop for Sunday residential pre-orders.  
* 2\. Updating Order Statuses:  
  * Roastery operators update Column P in Sheet1, Column U in B2B Orders, or Column L in Custom & Event Inquiries using the exact keywords listed in Section 6\.  
...
* 2\. Frontend Configuration (app.js):  
  * Set CONFIG.googleSheetEndpoint to your deployed Apps Script Web App URL (https://script.google.com/macros/s/.../exec).  
  * Set CONFIG.razorpayKeyId to your active Razorpay Key ID (rzp\_live\_... or rzp\_test\_...).  
...
* Core Web Vitals: Preconnect and dns-prefetch hints for Razorpay checkout, CSS containment (contain: content), GPU-accelerated transforms on progress bars, and debounced validation.  
* Offline Resilience & PWA: Service worker caching (sw.js) and automated localStorage retry queue (tabc\_pending\_orders) to prevent lost orders during momentary network interruptions.