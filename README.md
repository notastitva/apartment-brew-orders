...
\* Rule-Based Decision Matrix: Evaluates answer pairs (e.g., opt\_morning \+ opt\_berry \-\> LOT-01) with wildcard fallback (\*), styling the result box and generating direct checkout links.  
...
| :--- | :--- | :--- |  
| Store Status | \`OPEN\` | Global web checkout switch (\`OPEN\` / \`PAUSED\` / \`SOLD\_OUT\`) |  
...
\* \--- FLAVOR QUIZ QUESTIONS & OPTIONS \---  
   \* Question ID (Q1 / Q2) | Question Title | Option Key (opt\_morning, opt\_berry) | Option Icon (🌅, 🍇) | Option Label  
...
| P | Payment Status | Gateway capture reference (\`Paid via Gateway (pay\_XXX)\`) |  
...
| Q | Payment Status | \`Paid via Gateway (pay\_XXX)\` or \`Corporate Invoice\` |  
...
\* Ledger Audit Trail: The applied coupon code and exact discount amount are appended into Column R (Notes / UTR) of Sheet1 (e.g., Payment ID: pay\_TUQ9XEXPBWbtHR | Coupon: NCRFIRST (-₹90)).  
\---  
...
\* Frictionless Reorder Flow:  
   ...
   \* Captures the tracked order's bean lot, pack size, quantity, customer profile, address, and drop instructions in tabc\_refill\_order.  
...
\* Database & Tracking Integration:  
   \* Logged with \[STANDING\_ORDER: WEEKLY\] or \[STANDING\_ORDER: BI\_WEEKLY\] tags in Google Sheets.  
